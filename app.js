...
const Musica = {
    ...
    async search() {
        ...
        try {
            ...
            // Fallback to Piped search if Google API fails
            const res = await fetch(`${this.mirrors[0]}/search?q=${query}&filter=music_songs`);
            const data = await res.json();
    ...
    async loadTrending() {
        ...
        try {
            const res = await fetch(`${this.mirrors[0]}/trending?region=${region}`);
            const data = await res.json();
    ...
    async play(id, title, artist, thumb) {
        ...
        for (let mirror of this.mirrors) {
            ...
            } catch (e) { console.warn("Mirror failed, trying next..."); }
        }
        alert("Servers are busy. Try another song or wait a moment.");
    },
